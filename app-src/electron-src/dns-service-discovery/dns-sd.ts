  ...
  _checkDiscoveryParameters(params) {
    ...
    if ("name" in params) {
      ...
      if (typeof v === "string") {
        if (v === "") {
          return {
            error: new Error("The `name` must be an non-empty string."),
          };
        }
        p["name"] = [v];
        ...
        if (v.length === 0) {
          ...
          return {
            error: new Error("The `name` can include up to 255 elements."),
          };
        }
        let err = null;
        const list = [];
        for (let i = 0; i < v.length; i++) {
          if (typeof v[i] === "string" && v[i] !== "") {
            ...
            err = new Error(
              "The `name` must be an Array object including non-empty strings."
            );
            break;
          }
        }
        ...
        return {
          error: new Error("The `name` must be a string or an Array object."),
        };
      }
    } else {
  ...
  _addMembership() {
    for (const netif_address of this._netif_address_list) {
      try {
        ...
        console.log(
          `Catching error on address already in use: ${JSON.stringify(e)}`
        );
      }
    }
  }
